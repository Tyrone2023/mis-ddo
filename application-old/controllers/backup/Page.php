...
class Page extends CI_Controller
{
	...
	public function school_allocations(){
		...
		$result['school'] = $this->SGODModel->no_cond('schools');
	...
	public function hire()
	{
		...
		$this->db->query("update users set position='user', password='" . $hash . "', username='" . $id . "', user_id='" . $id . "' where username='" . $username . "'");
	...
	public function jobVacancy()
	{
		...
		if ($this->input->post('submit')) {
			...
			$que1 = $this->db->query("insert into hris_jobvacancy (jobTitle, empType, department, qty, description, postedBy, datePosted, jvStatus, itemNo) values('$jobTitle','$empType','$department','$qty','$description','$postedBy','$date','Open','$itemNo')");
	...
	public function archive_jv()
	{
		...
		$this->Page_model->insert_at('Archieved Job Vacancy.');
	...
	public function memo() {
		...
		if ($this->input->post('submit')) {
			...
			if($check->num_rows() <= 0){
			$config['allowed_types'] = 'pdf';
			$config['upload_path'] = './uploads/memo/';
			$this->load->library('upload', $config);
	...
	function aip_sub_school(){
		...
		$result['data'] = $this->SGODModel->two_cond('sgod_aip_submit', 'fy', '2024', 'school_id', $school_id);
	...
	function aip_new()
	{
		...
		$result['pias'] = $this->SGODModel->two_cond('sgod_settings_pias', 'year', $fys->fy, 'school_id', $this->session->username);
		...
		$result['ssa'] = $this->SGODModel->two_cond('sgod_school_allocation', 'schoolID', $this->session->username, 'alloc_year', $fys->fy);
	...
	function implementation_plans(){
		...
		$result['ssa'] = $this->SGODModel->two_cond('sgod_school_allocation', 'schoolID', $this->session->username, 'alloc_year', $fy->fy);
