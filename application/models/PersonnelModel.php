	...
	public function leaveCredits($currentMonth, $currentYear)
{
    ...
    // Filter by leaveCredits status, vlMonth, and vlYear
    $this->db->where('leaveCredits', "Yes");
    $this->db->where('vlMonth', $currentMonth);  // Filter for current month
    $this->db->where('vlYear', $currentYear);    // Filter for current year
	...
	public function leaveCreditsSummary()
{
	...
	public function leave_coc()
	{
		$this->db->select('leave_coc.*, hris_staff.*'); // Select fields from both tables
		$this->db->from('leave_coc'); // Primary table
		$this->db->join('hris_staff', 'hris_staff.IDNumber = leave_coc.IDNumber'); // Join condition
